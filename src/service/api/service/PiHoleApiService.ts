...
export class PiHoleApiService {
     ...
     */
    public async get_pi_hole_version(): Promise<PiHoleVersions[]> {
        const storage = (await this.get_storage_service().get_pi_hole_settings_array());
        if (typeof storage === "undefined") {
            return [];
        }
        let promise_array = [];
        for (const pi_hole_setting of storage) {
            promise_array.push(new Promise<PiHoleVersions>(resolve => this.pi_hole_version_promise_function(pi_hole_setting, resolve)));
        }
     ...
     * @param list
     */
    public async sub_domain_from_list(domain: string, list: ApiListMode): Promise<void> {
        ...
        for (const pi_hole of storage) {
            request_promises.push(new Promise<void>(resolve => this.sub_domain_from_list_promise_function(pi_hole, list, domain, resolve)));
        }
     ...
     * @param mode
     */
    public async list_domain(domain: string, mode: ApiListMode): Promise<PiHoleListStatus[]> {
        const request_promises = [];
        const storage = (await this.get_storage_service().get_pi_hole_settings_array());
        if (typeof storage === "undefined") {
            return [];
        ...
        for (const pi_hole of storage) {
            request_promises.push(new Promise<PiHoleListStatus>(resolve => this.list_domain_promise_function(pi_hole, mode, domain, resolve)));
        }
     ...
     * @param errorCallback
     */
    public async change_pi_hole_status(status: PiHoleApiStatusEnum, time: number, successCallback: (data: PiHoleApiStatus) => void, errorCallback: (data: string) => void): Promise<void> {
        const pi_hole_storage = (await this.get_storage_service().get_pi_hole_settings_array());
     ...
     */
    public async refresh_pi_hole_status(successCallback: (data: PiHoleApiStatus) => void): Promise<void> {
        const request_promises = [];
        const storage = (await this.get_storage_service().get_pi_hole_settings_array());
        if (typeof storage === "undefined") {
            return;
        ...
        for (const pi_hole of storage) {
            const promise_function = (resolve: (value?: any) => void) => this.refresh_pi_hole_status_promise_function(pi_hole, resolve);
     ...
     * @private
     */
    private sub_domain_from_list_promise_function(pi_hole_settings: PiHoleSettingsStorage, mode: ApiListMode, domain: string, resolve: () => void): void {
        const pi_uri_base = pi_hole_settings.pi_uri_base;
        const api_key = pi_hole_settings.api_key;
        if (typeof pi_uri_base === "undefined" || typeof api_key === "undefined") {
            return;
     ...
     * @param resolve
     */
    private list_domain_promise_function(pi_hole_settings: PiHoleSettingsStorage, mode: ApiListMode, domain: string, resolve: (value: PiHoleListStatus) => void): void {
        const pi_uri_base = pi_hole_settings.pi_uri_base;
        const api_key = pi_hole_settings.api_key;
        if (typeof pi_uri_base === "undefined" || typeof api_key === "undefined") {
     ...
     * @param resolve
     */
    private refresh_pi_hole_status_promise_function(pi_hole_settings: PiHoleSettingsStorage, resolve: (value: PiHoleApiStatus) => void): void {
        const pi_uri_base = pi_hole_settings.pi_uri_base;
        const api_key = pi_hole_settings.api_key;
        if (typeof pi_uri_base === "undefined" || typeof api_key === "undefined") {
