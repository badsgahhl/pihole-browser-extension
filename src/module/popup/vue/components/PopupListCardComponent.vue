<template>
  <b-card no-body>
    <b-card-header id="list_card" class="status">
    ...
    <b-card-footer class="text-center">
      ...
      >
        ...
        />
        <b-spinner v-else style="width:20px;height: 20px" />
      </b-button>
      <b-button
...
export default class PopupListCardComponent extends BaseComponent {
  ...
  private async list_domain(mode: ApiList): Promise<void> {
    ...
    const piHoleListResults = await PiHoleApiService.addDomainToList(
      mode,
      domain
    )
      ...
      setTimeout(() => {
        ...
        if (index + 1 === piHoleListResults.length) {
          setTimeout(async () => {
            ...
            if (
              typeof reloadAfterWhitelist !== 'undefined' &&
              reloadAfterWhitelist &&
              mode === ApiList.whitelist &&
              responseData.success &&
              responseData.message.includes('Added')
            ) {
